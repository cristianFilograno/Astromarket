fetch("./DATABASE.json")
    ...
    .then(data => {
        ...
        // FILTRA PER NUOVO USATO
        const nuovo = document.querySelector('#nuovo')
        const usato = document.querySelector('#usato')

        nuovo.addEventListener('click', () => {
            showCards(filterByNew(annunci))
         })

        usato.addEventListener('click', () => {
            showCards(filterByUsed(annunci))
         })
        ...
        // FILTRA PER PREZZO
        const low = document.querySelector('#lowPrice')
        const middle = document.querySelector('#midPrice')
        const high = document.querySelector('#highPrice')

        low.addEventListener('click', () => {
            showCards(filterByLow(annunci))
            
         })
        middle.addEventListener('click', () => {
            showCards(filterByMid(annunci))
         })
        high.addEventListener('click', () => {
            showCards(filterByHigh(annunci))
            
         })


        function filterByLow(array){
            return array.filter(el => el.price <= 1000)
            
        } 

        function filterByMid(array){
            return array.filter(el =>1000 < el.price <= 2000)
        }
            

        function filterByHigh(array){
            return array.filter(el => el.price > 2000)
        }
            
        


        // INIETTA LE CARD NELL'HTML
